-- ════════════════════════════════════════════════════════════════════════
-- Refresh provision_member's default starter app set (ADDITIVE — replaces fn)
-- ════════════════════════════════════════════════════════════════════════
-- Two changes to the default enabled_apps/app_order seeded for NEW member
-- nodes (existing members are untouched; they curate via Customize → Apps):
--   • add 'videos' (ChemTube) so members get their own video app by default
--   • drop 'about' and 'terminal' — both are now flagship-only (Eric's bespoke
--     README.sh + shell), so they never render on member nodes anyway.
-- Function body is otherwise identical to 20260601000000_provision_member.sql.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  values (
    ...
    'warm-slate',
    array[
      'aboutme','blog','pictures','videos','guestbook','music','reviews',
      'restaurants','wishlist','trips','digest','email','messageboard'
    ],
    array[
      'aboutme','blog','pictures','videos','guestbook','music','reviews',
      'restaurants','wishlist','trips','digest','email','messageboard'
